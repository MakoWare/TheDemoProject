...
/**
 *  A RubeSprite extends from {@link com.badlogic.gdx.graphics.g2d.PolygonSprite} with additional information to properly follow a {@link com.badlogic.gdx.physics.box2d.Body}
...
public class RubePolygonSprite extends PolygonSprite
{
	...
	/**
	 * Convenience method to create a new RubePolygonSprite from a {@link com.badlogic.gdx.physics.box2d.Fixture}. <br/>
	 * Can return null if the Fixture doesn't meet the requirement (not a polygon fixture or body is null).
	 * @param texture {@link com.badlogic.gdx.graphics.Texture} to use to draw the sprite.
	 * @param fixture Reference {@link com.badlogic.gdx.physics.box2d.Fixture} to create the sprite.
	...
	public static RubePolygonSprite createRubePolygonSprite(Texture texture, Fixture fixture)
	{
		if(texture != null && fixture != null && fixture.getBody() != null && fixture.getType() == Shape.Type.Polygon)
		{
			...
			if (body.getType() == BodyType.StaticBody)
			{
                ...
                short [] triangleIndices = new EarClippingTriangulator().computeTriangles(vertices).toArray();
				return new RubePolygonSprite(texture, vertices, triangleIndices);
			}
			else
			{
                ...
                short [] triangleIndices = new EarClippingTriangulator().computeTriangles(vertices).toArray();
				return new RubePolygonSprite(texture, vertices, triangleIndices, body);
			}
		}
		return null;
	}
	...
	public RubePolygonSprite(Texture texture, float[] vertices, short[] triangles) {
		this(new PolygonRegion(new TextureRegion(texture), vertices, triangles), null);
	}
	
	public RubePolygonSprite(Texture texture, float[] vertices, short[] triangles, Body body) {
		this(new PolygonRegion(new TextureRegion(texture), vertices, triangles), body);
	}
